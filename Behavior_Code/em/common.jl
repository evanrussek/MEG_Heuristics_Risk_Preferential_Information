...
# what's startx?
function optimizesubject_orig(likfun, startx)
...
function optimizesubject(likfun, startx)
	#a = optimize(likfun, startx, NewtonTrustRegion(); autodiff=:forward)
	# Evan: check that the likfun runs at startx, otherwise, get new start...
    
	#'if likfun(startx) > 1e8
#		startx = generate_start_vals(likfun, length(startx))
#	end
    try
        a = optimize(likfun, startx, LBFGS(); autodiff=:forward)
        return(a.minimum,a.minimizer)
        ...
        println("failure")
        a = optimize(likfun, startx, BFGS(linesearch=LineSearches.BackTracking()); autodiff=:forward)
    return(a.minimum,a.minimizer)
    end
